...
class PlayerActivity : AppCompatActivity(), ServiceConnection, MediaPlayer.OnCompletionListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (intent.data?.scheme.equals("content", ignoreCase = true)) {
            intent.data?.let { uri ->
                songPosition = 0
                ...
                try {
                    musicListPA = arrayListOf(getMusicDetails(uri))
                    Glide.with(this)
                    ...
                    finish() // Optionally finish the activity if media loading fails
                }
            } ?: run {
                Toast.makeText(this, "Invalid media URI", Toast.LENGTH_SHORT).show()
                finish() // Optionally finish the activity if URI is null
            }
        } else {
            initializeLayout()
        }

        // Playback Speed Button Click Listener
    ...
    private fun setPlaybackSpeed(speed: Float) {
        musicService?.mediaPlayer?.playbackParams = musicService?.mediaPlayer?.playbackParams?.setSpeed(speed)!!
        Toast.makeText(this, "Playback speed set to $speed", Toast.LENGTH_SHORT).show()
    }
//Important Function
